 ...
 */
@Slf4j
public class CaptureDataByRowCallable extends AbstractCaptureDataCallable {

    public CaptureDataByRowCallable(CaptureConditionDTO captureConditionDTO, CommonConfig commonConfig, Map<String, DataTypeConvert> dataTypeConvertMap, DataProducer dataProducer) {
    ...
    public void analyseDataFromResultSet(@NotNull ResultSet resultSet) throws SQLException {
        ...
        while (resultSet.next()) {
            ...
            if (StringUtils.isNotBlank(redoSql)) {
                ...
                if (rollback == 1) {
                    originDataDTO = generatorOriginDataDTO(taskId, captureTableDTO, lastScn, timestamp, OperationType.ROLLBACK);
                ...
                if (Objects.nonNull(originDataDTO) && Objects.nonNull(getLogger())) {
                    builder.append(getDataJson(originDataDTO)).append('\n');
                    synchronized (this) {
                        if (builder.length() >= 512000) {
        ...
        if (builder.length() > 0 && Objects.nonNull(getLogger())) {
            getLogger().info(builder.toString());
            builder.setLength(0);
        }
