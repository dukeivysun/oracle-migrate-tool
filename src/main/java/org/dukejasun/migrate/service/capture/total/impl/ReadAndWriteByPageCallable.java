 ...
 */
@Slf4j
public class ReadAndWriteByPageCallable extends ReadAndWriteCommonService implements Callable<Object> {
    ...
    public ReadAndWriteByPageCallable(String taskId, Long dataCount, Integer startIndex, Integer endIndex, Integer commitSize, String schemaName, String tableName, String destSql, String parentDir, CommonConfig commonConfig, DatasourceDTO targetSourceConfig, JdbcTemplate jdbcTemplate, OracleConnection oracleConnection, CaptureTableDTO captureTableDTO, List<CaptureColumnDTO> captureColumnDTOList, List<String> columnNameList, String columns, CyclicBarrier barrier, DatabaseFeatures databaseFeatures, DatabaseFeatures targetDatabaseFeatures, DataProducer dataProducer) {
        this.taskId = taskId;
    ...
    public Object call() throws Exception {
        String querySql = MessageFormat.format(IConstants.QUERY_TABLE_1, columnNameList.stream().map(databaseFeatures::getColumnName).collect(Collectors.joining(",")), schemaName, tableName, columns);
        ...
        try {
            if (StringUtils.isNotBlank(querySql)) {
                ...
                log.info("任务Id:【{}】完成{}.{}表({},{}]范围的迁移!", taskId, schemaName, tableName, startIndex, endIndex);
            ...
            migrateResultDTO.setErrorReason(MessageFormat.format(Constants.TOTAL_ERROR_MESSAGE, String.valueOf(startIndex), String.valueOf(endIndex), e.getMessage()));
